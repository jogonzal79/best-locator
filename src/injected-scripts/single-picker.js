(() => {
  ...
  class OverlayManager {
    ...
    createOverlay() {
      ...
      this.overlay.id = 'bestlocator-overlay-single';
      this.overlay.setAttribute('data-best-locator', 'single-overlay');
  ...
  const highlightElement = (event) => {
    ...
    if (!target || target.hasAttribute('data-best-locator') || target.id === 'bestlocator-overlay-single') {
  ...
  const clickHandler = (event) => {
    ...
    if (target.hasAttribute('data-best-locator') || target.id === 'bestlocator-overlay-single') {
      return;
    }
    ...
    window.selectedElementInfo = gatherInfo(elementData);
    window.elementSelected = true;
    
    // Visual feedback - same as toggle mode flash
    if (elementData && elementData.element) {
      const rect = elementData.element.getBoundingClientRect();
      
      // Flash effect like toggle mode
